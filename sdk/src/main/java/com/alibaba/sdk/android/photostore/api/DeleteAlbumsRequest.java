...
public class DeleteAlbumsRequest extends BaseRequest {
    ...
    public DeleteAlbumsRequest(List<Long> ids) {
        this.ids = ids;
    }

    @Override
    public RpcAcsRequest build() {
        com.aliyuncs.cloudphoto.model.v20170711.DeleteAlbumsRequest request = new com.aliyuncs.cloudphoto.model.v20170711.DeleteAlbumsRequest();
        ...
        request.setAlbumIds(ids);
        if (!stsToken.isEmpty())
            request.setSecurityToken(stsToken);
        else if (!libraryId.isEmpty())
            request.setLibraryId(libraryId);
        request.setStoreName(storeName);
        request.setActionName("DeleteAlbums");
        return request;
    }

    @Override
    public BaseResponse parse(String result) {
        return JSON.parseObject(result, DeleteAlbumsResponse.class);
    }
}
