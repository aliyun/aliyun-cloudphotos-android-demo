...
public class RenameAlbumRequest extends BaseRequest {
    private Long id;
    private String name;

    public RenameAlbumRequest(Long id, String name) {
        this.id = id;
        this.name = name;
    }
    ...
    public RpcAcsRequest build() {
        com.aliyuncs.cloudphoto.model.v20170711.RenameAlbumRequest request = new com.aliyuncs.cloudphoto.model.v20170711.RenameAlbumRequest();
        ...
        request.setAlbumName(name);
        if (!stsToken.isEmpty())
            request.setSecurityToken(stsToken);
        else if (!libraryId.isEmpty())
            request.setLibraryId(libraryId);
        request.setStoreName(storeName);
        request.setActionName("RenameAlbum");
        return request;
    }

    @Override
    public BaseResponse parse(String result) {
        return JSON.parseObject(result, RenameAlbumResponse.class);
    }
}
