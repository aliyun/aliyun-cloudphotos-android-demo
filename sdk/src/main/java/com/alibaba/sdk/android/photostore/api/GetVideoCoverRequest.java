...
public class GetVideoCoverRequest extends BaseRequest {
    private Long photoId;
    private String zoomType;

    public GetVideoCoverRequest(Long photoId, String zoomType) {
    ...
    public RpcAcsRequest build() {
        com.aliyuncs.cloudphoto.model.v20170711.GetVideoCoverRequest request = new com.aliyuncs.cloudphoto.model.v20170711.GetVideoCoverRequest();
        ...
        request.setZoomType(zoomType);
        if (!stsToken.isEmpty())
            request.setSecurityToken(stsToken);
        else if (!libraryId.isEmpty())
            request.setLibraryId(libraryId);
        request.setStoreName(storeName);
        request.setActionName("GetVideoCover");
        return request;
    }

    @Override
    public BaseResponse parse(String result) {
        return JSON.parseObject(result, GetVideoCoverResponse.class);
    }
}
