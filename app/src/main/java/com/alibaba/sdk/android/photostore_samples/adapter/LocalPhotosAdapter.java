...
public class LocalPhotosAdapter extends HeaderFooterAdapter {
    ...
    public LocalPhotosAdapter(Context context, int gridCount) {
    ...
    protected void onBindContentViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof PhotoViewHolder) {
            final File f = datas.get(position);
            final PhotoViewHolder h = (PhotoViewHolder) holder;

            h.position = position;

            Glide.with(mContext.getApplicationContext()).load(Uri.fromFile(f)).into(h.ivPhoto);
